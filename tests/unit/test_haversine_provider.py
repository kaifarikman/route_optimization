...
class TestHaversineRoutingProvider:
    ...
    def three_points(self):
        return [{"lat": 55.7558, "lon": 37.6173}, {"lat": 55.7589, "lon": 37.6231}, {"lat": 55.7570, "lon": 37.6200}]
    ...
    def test_three_points_distance_is_sum_of_segments(self, provider, three_points):
        ...
        segment1 = haversine((three_points[0]["lat"], three_points[0]["lon"]),
                             (three_points[1]["lat"], three_points[1]["lon"]))
        segment2 = haversine((three_points[1]["lat"], three_points[1]["lon"]),
                             (three_points[2]["lat"], three_points[2]["lon"]))
        expected_total = segment1 + segment2
        assert result["distance_km"] == pytest.approx(expected_total, rel=1e-6)

    def test_works_without_internet(self, provider, sample_points, monkeypatch):
        def block_network(*args, **kwargs):
            raise RuntimeError("Network blocked - haversine should not need network")
