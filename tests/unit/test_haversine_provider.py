...
class TestHaversineRoutingProvider:
    ...
    def three_points(self):
        return [
            Point(id=1, lat=55.7558, lon=37.6173),
            Point(id=2, lat=55.7589, lon=37.6231),
            Point(id=3, lat=55.7570, lon=37.6200),
        ]
    ...
    def test_duration_on_40kmh(self, provider, sample_points):
        result = provider.build_route(sample_points)
        expected_duration_minutes = (result.distance_km / 40.0) * 60.0
        assert result.duration_minutes == pytest.approx(expected_duration_minutes, rel=1e-6)
        assert 1.6 <= result.duration_minutes <= 2.2

    def test_three_points_distance_is_sum_of_segments(self, provider, three_points):
        result = provider.build_route(three_points)
        seg1 = calculate_distance(
            three_points[0].lat, three_points[0].lon,
            three_points[1].lat, three_points[1].lon,
        )
        seg2 = calculate_distance(
            three_points[1].lat, three_points[1].lon,
